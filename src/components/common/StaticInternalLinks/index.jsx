...
import './static-internal-links.css'
...
const StaticInternalLinks = ({
  ...
  const content = linksData[language] || linksData.en
  ...
  return (
    <section className={`static-internal-links-section static-internal-links-section--${placement}`}>
      <div className="container">
        <div className="static-internal-links-header">
          <h2 className="static-internal-links-title">{content.title}</h2>
          <p className="static-internal-links-subtitle">{content.subtitle}</p>
        </div>

        <div className="static-internal-links-grid">
          {content.links.map((link, index) => (
            <Link
              key={index}
              to={createLocalizedPath(link.path, language)}
              className="static-internal-link-card"
              aria-label={`${link.title}: ${link.description}`}
            >
              <div className="static-internal-link-icon">
                {renderIcon(link.icon)}
              </div>
              <div className="static-internal-link-content">
                <h3 className="static-internal-link-title">{link.title}</h3>
                <p className="static-internal-link-description">{link.description}</p>
              </div>
              <div className="static-internal-link-arrow">
...
StaticInternalLinks.displayName = 'StaticInternalLinks'

export default StaticInternalLinks